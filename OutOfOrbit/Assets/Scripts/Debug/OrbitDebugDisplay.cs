...
public class OrbitDebugDisplay : MonoBehaviour
{
    ...
    [Tooltip("Only works properly if relativeToBody is false (or somewhat if centralBody moves very slowly).")]
    public bool showLinesDuringPlayMode;

    void Start()
    {
        if (Application.isPlaying && !showLinesDuringPlayMode)
        {
            HideOrbits();
        }
    }
